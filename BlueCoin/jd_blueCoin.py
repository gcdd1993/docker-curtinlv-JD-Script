...
def set_headers(cookie):
    ...
    headers = {
        ...
        'Host': 'api.m.jd.com',
        'User-Agent': 'jdapp;iPhone;9.4.8;14.3;809409cbd5bb8a0fa8fff41378c1afe91b8075ad;network/wifi;ADID/201EDE7F'
                      '-5111-49E8-9F0D-CCF9677CD6FE;supportApplePay/0;hasUPPay/0;hasOCPay/0;model/iPhone13,'
                      '4;addressid/2455696156;supportBestPay/0;appBuild/167629;jdSupportDarkMode/0;Mozilla/5.0 ('
...
def getBlueCoinInfo(headers):
    try:
        url = 'https://api.m.jd.com/api?appid=jdsupermarket&functionId=smtg_newHome&clientVersion=8.0.0&client=m&body=%7B%22channel%22:%2218%22%7D&t={0}'.format(
            timestamp)
...
def getAllUserInfo():
    id_num = 1
    for ck in cookies:
        headers, userName = set_headers(ck)
        try:
            totalBlue, shopName = getBlueCoinInfo(headers)
            url = 'https://api.m.jd.com/api?appid=jdsupermarket&functionId=smtg_receiveCoin&clientVersion=8.0.0&client=m&body=%7B%22type%22:4,%22channel%22:%2218%22%7D&t={0}'.format(
                timestamp)
            respon = requests.get(url=url, verify=False, headers=headers)
            result = respon.json()
            level = result['data']['result']['level']
            print("【用户{4}:{5}】: {0} {3}\n【等级】: {1}\n【蓝币】: {2}万\n------------------".format(shopName, level,
                                                                                           totalBlue / 10000, totalBlue,
                                                                                           id_num, userName))
        except Exception as e:
            # print(e)
...
def smtg_queryPrize(headers, coinToBeans):
    url = 'https://api.m.jd.com/api?appid=jdsupermarket&functionId=smt_queryPrizeAreas&clientVersion=8.0.0&client=m&body=%7B%22channel%22:%2218%22%7D&t={}'.format(
        timestamp)
    try:
...
def smtg_obtainPrize(prizeId, headers):
    timestamp = int(round(time.time() * 1000))
    url = 'https://api.m.jd.com/api?appid=jdsupermarket&functionId=smtg_obtainPrize&clientVersion=8.0.0&client=m&body=%7B%22prizeId%22:%22{0}%22,%22channel%22:%221%22%7D&t={1}'.format(
        prizeId, timestamp)
    try:
...
def checkUser(cookies):  # 返回符合条件的ck list
    ...
    user_num = 1
    for i in cookies:
        headers, userName = set_headers(i)
        try:
            totalBlue, shopName = getBlueCoinInfo(headers)
            title, prizeId, blueCost, inStock, skuId = isCoinToBeans(coinToBeans, headers)
